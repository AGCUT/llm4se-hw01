...
export const Register = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // 检查是否有 session（如果禁用了邮箱验证，注册后会立即有 session）
      const { user, session } = useAuthStore.getState()
      
      console.log('注册后状态检查:', { hasUser: !!user, hasSession: !!session })
      
      if (session && user) {
        // 有 session，说明已自动登录，直接跳转到首页
        console.log('注册成功，已自动登录，跳转到首页')
        navigate('/')
      } else {
        // 没有 session，说明需要邮箱验证，跳转到登录页
        console.log('注册成功，需要邮箱验证，跳转到登录页')
        alert('🎉 注册成功！\n\n如果启用了邮箱验证：\n1. 请检查您的邮箱（包括垃圾箱）\n2. 点击验证链接\n3. 返回登录\n\n如果禁用了邮箱验证：\n可以直接登录')
        navigate('/login')
      }
    } catch (error: any) {
