...
| 类别 | 技术选型 | 版本 | 用�?|
|------|---------|------|------|
| **UI框架** | React | 18.2+ | 组件化开�?|
| **构建工具** | Vite | 5.0+ | 快速构建、HMR |
| **路由** | React Router | 6.20+ | 页面路由管理 |
| **状态管�?* | Zustand | 4.4+ | 轻量级状态管�?|
| **数据请求** | TanStack Query | 5.0+ | 数据获取、缓�?|
| **UI组件�?* | Ant Design | 5.12+ | 基础组件 |
| **样式方案** | Tailwind CSS | 3.4+ | 原子化CSS |
| **地图SDK** | 高德地图 JS API | 2.0 | 地图展示、导�?|
| **语音识别** | Web Speech API / 讯飞WebSocket | - | 语音转文�?|
| **图表�?* | ECharts | 5.4+ | 数据可视�?|
| **表单管理** | React Hook Form | 7.49+ | 表单验证 |
| **日期处理** | Day.js | 1.11+ | 日期格式�?|
| **HTTP客户�?* | Axios | 1.6+ | API请求 |
| **实时通信** | Socket.io-client | 4.6+ | WebSocket连接 |
| **动画�?* | Framer Motion | 10.16+ | 流畅动画 |
| **国际�?* | i18next | 23.7+ | 多语言支持 |
...
| 类别 | 技术选型 | 版本 | 用�?|
|------|---------|------|------|
| **运行�?* | Node.js | 20 LTS | JavaScript运行环境 |
| **Web框架** | Express | 4.18+ | HTTP服务�?|
...
| **邮件** | Nodemailer | 6.9+ | 发送邮�?|
| **WebSocket** | Socket.io | 4.6+ | 实时通信 |
| **缓存** | ioredis | 5.3+ | Redis客户�?|
...
| 类别 | 技术选型 | 用�?|
|------|---------|------|
| **主数据库** | PostgreSQL 15 | 用户数据、行程数据、费用记�?|
| **缓存�?* | Redis 7.2 | Session、热点数据缓�?|
| **对象存储** | 阿里云OSS / AWS S3 | 用户头像、行程图�?|
| **数据库服�?* | Supabase | 提供PostgreSQL + Auth + Storage |

### 2.4 第三方服务集�?

| 服务类别 | 选型方案 | SDK/API | 用�?|
|---------|---------|---------|------|
| **AI大模�?* | 阿里云通义千问 | HTTP API | 行程生成、智能问�?|
| 备�?| OpenAI GPT-4 | openai SDK | 国际用户 |
| 备�?| DeepSeek | HTTP API | 成本优化 |
| **地图服务** | 高德地图 | JavaScript API 2.0 | 地图展示、路线规�?|
| 备�?| 百度地图 | JavaScript API 3.0 | 备用方案 |
| **语音识别** | 科大讯飞 | WebSocket API | 实时语音识别 |
| 备�?| 阿里云语�?| HTTP API | 语音转文�?|
| 备�?| Web Speech API | 浏览器原�?| 免费但功能有�?|
| **短信服务** | 阿里云SMS | HTTP API | 验证码发�?|
| **邮件服务** | SendGrid / 阿里邮件推�?| SMTP/API | 邮件通知 |
...
| **容器�?* | Docker 24+ | 应用容器�?|
| **容器编排** | Docker Compose | 本地开发环�?|
| **CI/CD** | GitHub Actions | 自动化构建部�?|
| **镜像仓库** | 阿里云容器镜像服�?| 镜像存储 |
...
```javascript
{
  "语音输入": {
    ...
    "流程": "麦克�?�?MediaRecorder �?WebSocket �?讯飞API �?文字结果"
  },
  "文本输入": {
    "前端": [
...
export const BudgetChart = ({ budgetData }) => {
  const option = {
    ...
    series: [{
      ...
      radius: '50%',
      data: [
        { value: budgetData.transportation, name: '交�? },
...
export const authenticate = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!token) {
    return res.status(401).json({ error: '未提供认证令�? })
  }
  ...
  try {
    ...
    if (error.name === 'TokenExpiredError') {
      return res.status(401).json({ error: '令牌已过�? })
    }
    return res.status(403).json({ error: '无效的令�? })
  }
}
```
...
```javascript
{
  ...
  "实时同步": {
    "方案1": "Supabase Realtime (PostgreSQL触发�?",
    "方案2": "Socket.io (自定义实�?",
    "冲突解决": "Last-Write-Wins + 版本�?
  },
  "离线支持": {
...
├── src/
�?  ├── controllers/                 # 控制器层
�?  �?  ├── auth.controller.ts       # 认证控制�?
�?  �?  ├── user.controller.ts       # 用户控制�?
�?  �?  ├── trip.controller.ts       # 行程控制�?
�?  �?  ├── expense.controller.ts    # 费用控制�?
�?  �?  └── voice.controller.ts      # 语音控制�?
�?  �?
�?  ├── services/                    # 业务逻辑�?
�?  �?  ├── auth.service.ts
�?  �?  ├── user.service.ts
...
�?  �?
�?  ├── middleware/                  # 中间�?
�?  �?  ├── auth.middleware.ts       # 认证中间�?
�?  �?  ├── validate.middleware.ts   # 验证中间�?
�?  �?  ├── error.middleware.ts      # 错误处理中间�?
�?  �?  ├── logger.middleware.ts     # 日志中间�?
�?  �?  ├── rateLimit.middleware.ts  # 限流中间�?
�?  �?  └── upload.middleware.ts     # 文件上传中间�?
�?  �?
�?  ├── validators/                  # 数据验证
�?  �?  ├── auth.validator.ts
�?  �?  ├── trip.validator.ts
...
�?  �?  ├── logger.util.ts           # 日志工具
�?  �?  ├── response.util.ts         # 响应格式�?
�?  �?  └── constants.ts             # 常量
�?  �?
�?  ├── config/                      # 配置文件
�?  �?  ├── database.config.ts       # 数据库配�?
...
�?  �?
�?  ├── prompts/                     # AI提示词模�?
�?  �?  ├── itinerary.prompt.ts      # 行程生成提示�?
�?  �?  ├── budget.prompt.ts         # 预算分析提示�?
�?  �?  └── qa.prompt.ts             # 问答提示�?
�?  �?
�?  ├── websocket/                   # WebSocket处理
�?  �?  ├── socketServer.ts          # Socket服务�?
�?  �?  ├── handlers/
�?  �?  �?  ├── trip.handler.ts
�?  �?  �?  └── voice.handler.ts
�?  �?  └── middleware/
...
model Expense {
  ...
  // 支付信息
  paymentMethod String?  // 现金/支付�?微信/信用�?
  payer         String?
...
| 优化�?| 方案 | 工具/技�?|
|--------|------|----------|
| **代码分割** | 路由懒加载、动态导�?| React.lazy, Suspense |
| **资源优化** | 图片压缩、WebP格式 | tinypng, imagemin |
| **缓存策略** | Service Worker、CDN | Workbox |
| **打包优化** | Tree Shaking、压�?| Vite, Terser |
| **首屏加载** | SSR、预渲染 | Next.js |
| **骨架�?* | 加载占位 | React Content Loader |

### 10.2 后端优化

| 优化�?| 方案 | 工具/技�?|
|--------|------|----------|
| **数据库查�?* | 索引优化、连接池 | Prisma, pg-pool |
