...
1. 数据库触发器可能没有正确创建
2. RLS 策略可能阻止了查询
3. profiles 表可能没有正确设置

---

## ✅ 解决方案

### 步骤 1: 在 Supabase SQL Editor 中执行以下 SQL

登录 Supabase Dashboard → SQL Editor → 新建查询，执行以下 SQL：
...
我还建议修改代码以更好地处理这个错误。让我更新 `getProfile` 函数：
...
2. **API 密钥正确**
   - 检查 `.env.local` 中的配置
   - 确保使用的是 `anon` key，不是 `service_role`
