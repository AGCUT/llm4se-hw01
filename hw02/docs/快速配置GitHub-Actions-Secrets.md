...
- **Secret**: 您的命名空间名称（例如：`your-username` �?`your-org`�?
  - 获取方式：登�?[阿里云容器镜像服务](https://www.aliyun.com/product/acr) �?实例列表 �?命名空间
  - 如果还没有命名空间，请先创建一�?
- 点击 **Add secret**
...
```bash
# 登录阿里云容器镜像服�?
# �?YOUR_ACCESS_KEY_ID �?YOUR_ACCESS_KEY_SECRET 替换为您的实际�?
docker login registry.cn-hangzhou.aliyuncs.com \
  -u YOUR_ACCESS_KEY_ID \
  -p YOUR_ACCESS_KEY_SECRET

# 如果登录成功，您会看�?"Login Succeeded"
```
...
4. **GitHub Secrets 安全**�?
   - GitHub Secrets 是加密存储的，只有有权限的用户才能查�?
   - 不要在代码、文档或日志中暴�?AccessKey Secret
