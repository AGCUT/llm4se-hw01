...
1. 在应用详情页，找到 **安全密钥** 或 **Security JS Code**
...
在 `frontend/index.html` 或 `frontend/public/index.html` 中添加：
...
// 返回: { lng: 116.397428, lat: 39.90923 } 或 null
```
...
const MyComponent = () => {
  ...
  return (
    <div>
      ...
      {!isConfigured && <div>请配置高德地图 API Key</div>}
...
- 检查安全密钥是否正确配置
- 确认在 HTML 中配置了 `window._AMapSecurityConfig`
...
在高德开放平台控制台中，为你的 Key 设置白名单：
- 开发环境：可以留空或添加 `localhost`
- 生产环境：添加你的域名，如 `https://yourdomain.com`
