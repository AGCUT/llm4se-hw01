...
| 提供商 | 推荐模型 | 获取地址 |
|--------|---------|---------|
| 阿里云百炼 | qwen-turbo | https://bailian.console.aliyun.com/ |
...
**原因：**  
`/settings` 路由被配置为**受保护路由**，需要登录后才能访问。

**查看路由配置：**
```typescript
// frontend/src/router.tsx
{
  element: <ProtectedRoute />,  // 需要登录
  children: [
    {
      path: 'settings',  // ← 这里
      element: <Settings />
...
**解决方案：**
现在不需要访问 `/settings` 了！直接在首页或导航栏配置即可。
